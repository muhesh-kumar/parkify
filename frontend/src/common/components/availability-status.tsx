...
import {
  getFractionalPartFromPercentage,
  getIntegralPartFromPercentage,
  getPercentageOfAvailableSlots,
          ...
          <ProgressProvider valueStart={0} valueEnd={availableSlots.size}>
            {(value: number) => (
              <CircularProgressbarWithChildren
                value={getPercentageOfAvailableSlots(value)}
                strokeWidth={3}
                styles={{
              ...
              >
                {!hover ? (
                  ...
                  <>
                    <h1 className="font-bold text-[30px]">
                      {getIntegralPartFromPercentage(
                        getPercentageOfAvailableSlots(value)
                      )}
                    ...
                    <h1 className="opacity-75 text-[15px]">Availability</h1>
                  </>
                )}
              </CircularProgressbarWithChildren>
            )}
